"""Tests for game.puzzles. Run from backend/:
...
# Make `src/` importable so `from game.puzzles import ...` works.
SRC = Path(__file__).resolve().parent.parent / "src"
sys.path.insert(0, str(SRC))

from game import puzzles  # noqa: E402
from game.puzzles import (  # noqa: E402
    GATE_MAP,
    INIT_PREP,
    N_QUBITS,
    PUZZLES_EASY,
...
class TestBuildCircuit(unittest.TestCase):
    ...
    def test_pre_measurement_basis_change(self):
        # |+> goal — circuit should apply H before measuring.
        p = Puzzle("|0>", "|+>", ["H"], ["H"], 1, 3, ["H"], "0")
        qasm = build_circuit(p, ["H"])
        # Two H gates total: one from the player, one from gates_before_measurement.
    ...
    def test_player_gates_appear_before_pre_measurement(self):
        p = Puzzle("|0>", "|+>", ["H"], ["H"], 1, 3, ["H"], "0")
        qasm = build_circuit(p, ["H"])
        # both H lines belong to q[0]; the player's H must come before the basis-change H.
        first = qasm.index("h q[0];")
        last = qasm.rindex("h q[0];")
        self.assertLess(first, last)
        # And the basis-change H must come before the measurement.
        self.assertLess(last, qasm.index("measure"))


class TestEvaluate(unittest.TestCase):
    """`evaluate` is mocked at the Quokka boundary so we control the outcome."""

    def _patch(self, counts):
        return patch.object(puzzles, "send_to_quokka", return_value=counts)
    ...
    def test_two_qubit_bell_state(self):
        p = next(pz for pz in PUZZLES_HARD if pz.goal_state.startswith("(|00> + |11>)") and pz.initial_state == "|00>")
...
class TestPools(unittest.TestCase):
    """Every puzzle in the pool must build a valid circuit from its declared
    solution, and every gate in the palette must parse."""
...
class TestRolling(unittest.TestCase):
    def test_roll_puzzle_stamps_metadata(self):
        ...
        self.assertEqual(p.shots, puzzles.DEFAULT_SHOTS)
...
class TestPayload(unittest.TestCase):
    def test_payload_has_public_fields_only(self):
        ...
        # Public fields the UI needs.
        for k in ("puzzle_id", "n_qubits", "initial_state",
                  "goal_state", "available_gates", "min_gates", "max_gates", "note"):
            self.assertIn(k, payload)
        # Solution must NOT leak to the client.
