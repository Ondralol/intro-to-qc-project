...
# Simplified version of game state, just represents which qubits have collapsed and their value
# Imagine that q0 and q1 have collapsed already and their value is 1 for this example
game_state = {
    "q0": {"collapsed": True,  "value": "1"},
    "q1": {"collapsed": True,  "value": "1"}, 
    "q2": {"collapsed": False, "value": None}, 
    "q3": {"collapsed": False, "value": None}, 
}
...
def build_radar_circuit(game_state: dict, scan_qubits: list[int]) -> str:
    ...
    Classical game state controls circuit structure:
      ...
      - Uncollapsed qubit: apply H gate to recreate superposition and CNOT to entangle pairs
    """
    ...
    for i, qubit in enumerate(qubits):
        ...
        if state["collapsed"]:
            ...
            # Quantum path: qubit still in superposition — recreate it
            if (i % 2 == 1): # Only apply to control qubit
                continue
            qasm += f"h q[{i}];  // {qubit} in superposition — applying H\n"
...
def run_radar(
    ...
    """
    Build radar circuit from current classical game state, execute it on Quokka and interpret results"""
